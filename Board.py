...
class GameResult(Enum):
    """
    Enum to encode different states of the game.
    A game can be in progress (NOT_FINISHED), lost, won, or draw
    """
    NOT_FINISHED = 0
    NAUGHT_WIN = 1
    CROSS_WIN = 2
...
#
# Values to encode the current content of a field on the board.
# A field can be empty, contain a naught, or contain a cross
#
EMPTY = 0  # type: int
NAUGHT = 1  # type: int
CROSS = 2  # type: int
...
#
# Define the length and width of the board. Has to be 3 at the moment,
# or some parts of the code will break.
# Also, the game mechanics kind of require this dimension
# unless other rules are changed as well.
# Encoding as a variable to make the code more readable
#
...
class Board:
    """
    The class to encode a tic-tac-toe board,
    including its current state of pieces.
    Also contains various utility methods.
    """

    #
    # We will use these starting positions and directions when checking
    # if a move resulted in the game being won by one of the sides.
    #
    ...
    def __init__(self, s=None):
        """
        Create a new Board.
        If a state is passed in, we use that otherwise we initialize with an empty board
    ...
    def move(self, position: int, side: int) -> (np.ndarray, GameResult, bool):
        """
        Places a piece of side "side" at position "position".
        The position is to be provided as 1D.
        *Throws a ValueError if the position is not EMPTY
        -> Should also check if the position is in the board : use is_legal...
        ...
        # is_legal was coded : let's use it...
    ...
    def __str__(self) -> str:
        ...
        for i in range(3):
            # Line by line
            board_str += self.state_to_char(i * 3) \
                         + '|' + \
