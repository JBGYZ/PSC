...
class TQPlayer(Player):
    ...
    def get_q(self, board_hash: int) -> [int]:
        ...
        # -> Many board states will never be reached during play
        # -> However, if we want to duplicate a trained agent, some positions will already be filled
        #
    ...
    def copy(self):
        # The crucial element is the table `self.q`
        return TQPlayer(alpha=self.learning_rate, gamma=self.value_discount, q_init=self.q_init_val, q=self.q.copy())

    # Additional import/export functions to save a trained table
    def export_TQ(self, file="TQPlayer.txt"):
        f = open(file,"w")
        f.write(str(self.q))
        f.close()

    def import_TQ(self, file="TQPlayer"):
        with open(file) as f:
            for line in f:
                (key, val) = line.split()
                self.q[int(key)] = val
